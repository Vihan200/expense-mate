      ...
      <Container maxWidth="md" sx={{ mt: 4 }}>
        <Paper elevation={3} sx={{ p: 4 }}>
          ...
          <Stack direction="row" spacing={2} mt={2}>
            {isAdmin && (
              <Button variant="contained" color="success" onClick={finalizeGroup}>
                Finalize Group
              </Button>
            )}
            {isAdmin && (
              <Button variant="outlined" startIcon={<AddIcon />} onClick={() => setOpenAddMember(true)}>
                Add Member
              </Button>
            )}
            <Button variant="outlined" startIcon={<AddIcon />} onClick={() => setOpenAddExpense(true)}>
              Add Expense
            </Button>
          </Stack>
          ...
          <Stack direction="row" flexWrap="wrap" gap={1} mt={1}>
            {group.members.map((member) => (
              <Chip key={member.uid} icon={<PersonIcon />} label={`${member.name} (${member.email})`} />
            ))}
          </Stack>
